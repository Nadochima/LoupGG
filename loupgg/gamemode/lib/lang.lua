...
local function resolve_path(dict,path,t,k)
  ...
  if el ~= nil then
    if type(el) == "table" then -- table, continue 
      return setmetatable({}, { __index = function(t,k) return resolve_path(el,path,t,k) end, __tostring = function(t) return path end, __call = function(t,...) return path end })
    else -- value
      return setmetatable({}, { __index = function(t,k) return resolve_path(el,path,t,k) end, __tostring = function(t) return el end, __call = function(t,...) return replace_args(el,{...}) end })
    end
